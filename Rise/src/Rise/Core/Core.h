...
// DLL support
	...
	#define RS_ASSERT(x, ...) { if(!(x)) { RS_ERROR("Assertion Failed: {0}", __VA_ARGS__); DEBUG_BREAK; } } static_assert(true, "Require semi-colon")
	#define RS_CORE_ASSERT(x, ...) { if(!(x)) { RS_CORE_ERROR("Assertion Failed: {0}", __VA_ARGS__); DEBUG_BREAK; } } static_assert(true, "Require semi-colon")
#else
