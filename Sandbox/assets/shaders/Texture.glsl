...
uniform sampler2D u_Textures[32]; // 32 max texture slots
...
void main()
{
	...
	switch(int(v_TexIndex)) // We do this instead of the above due to "When aggregated into arrays within a shader, they can only be indexed with a dynamically uniform integral expression, otherwise results are undefined." causes weird texture glithces
	{
